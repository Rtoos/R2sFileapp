...
public class R2sLib {
	...
	{ 
		try {
				...
				InputStream stream = classLoader.getResourceAsStream("../R2sLibConfiguration.cfg");
				if (stream == null) {
	...
	private String R2s_FlowService(String serviceid, String serviceurl, String serviceparam, String servicetype) throws IOException
	{
		R2s_IsValidJson(); 
		  try 
		  {
			  ...
			  newservice.put("service_type", servicetype);
/* $$$ to remove		
			  newservice.put("tries", R2s_TRIES);
			  newservice.put("timeout", R2s_TIMEOUT);
			  newservice.put("timeoutwait", R2s_TIMEOUT_WAIT);
*/			  
			  //newrequest.put("type", "Batch");
			  r2regarray.put(newservice);
